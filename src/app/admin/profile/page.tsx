...
import { Save, User, Phone, Mail, Linkedin, MapPin, Eye, Target, CheckCircle, AlertCircle, Upload } from "lucide-react";
...
const DEFAULTS: ProfileData = {
  vision_fr: "Technicien électromécanique industriel avec une expertise terrain acquise dans des environnements de production exigeants.",
...
export default function AdminProfilePage() {
  ...
  const handleSave = async () => {
    ...
    const { error } = profileId
      ? await sb.from("profile").update(payload).eq("id", profileId)
      : await sb.from("profile").insert(payload);
    setStatus(error ? "error" : "success");
    setSaving(false);
    if (!error) { await fetchProfile(); setTimeout(() => setStatus("idle"), 3000); }
  };
    ...
    <AdminLayout>
      <div className="max-w-3xl mx-auto">
        <div className="mb-8">
          ...
          <h1 className="heading-xl" style={{ color: "white", fontFamily: "var(--font-manrope)" }}>Profil & Contact</h1>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="p-6 rounded-2xl" style={{ background: "rgba(13,21,37,0.8)", border: "1px solid rgba(0,119,255,0.15)" }}>
            <div className="flex items-center gap-2 mb-5">
              ...
              <span className="label-tag ml-2" style={{ color: "#475569" }}>Apparaît dans le héro de la page d&apos;accueil</span>
            </div>

            <FileUpload
              bucket="images"
              storagePath="portrait/portrait.jpg"
              accept="image/jpeg,image/jpg,image/png,image/webp"
              maxSizeMB={5}
              currentUrl={form.portrait_url || null}
              label="Cliquer ou glisser votre photo portrait"
              hint="JPG, PNG, WebP · Format portrait recommandé (4:5) · Max 5 MB"
              accentColor="#22C55E"
              previewType="image"
              onUploadComplete={(url) => setForm(f => ({ ...f, portrait_url: url }))}
            />
          </div>
          ...
          <div className="p-6 rounded-2xl" style={{ background: "rgba(13,21,37,0.8)", border: "1px solid rgba(0,119,255,0.15)" }}>
            ...
            <div className="space-y-4">
              <div>
                ...
                <textarea rows={5} className={TA} style={IS} value={form.vision_fr} onChange={(e) => setForm({ ...form, vision_fr: e.target.value })} />
              </div>
              <div>
                <label className="label-tag block mb-2" style={{ color: "#475569" }}>VISION (ENGLISH)</label>
                <textarea rows={5} className={TA} style={IS} value={form.vision_en} onChange={(e) => setForm({ ...form, vision_en: e.target.value })} />
              </div>
            </div>
          </div>
          ...
          <div className="p-6 rounded-2xl" style={{ background: "rgba(13,21,37,0.8)", border: "1px solid rgba(0,119,255,0.15)" }}>
            ...
            <div className="space-y-4">
              <div>
                ...
                <textarea rows={3} className={TA} style={{ ...IS, lineHeight: 1.6 }} value={form.objectives_fr} onChange={(e) => setForm({ ...form, objectives_fr: e.target.value })} />
              </div>
              <div>
                <label className="label-tag block mb-2" style={{ color: "#475569" }}>OBJECTIVES (ENGLISH)</label>
                <textarea rows={3} className={TA} style={{ ...IS, lineHeight: 1.6 }} value={form.objectives_en} onChange={(e) => setForm({ ...form, objectives_en: e.target.value })} />
              </div>
            </div>
          </div>
